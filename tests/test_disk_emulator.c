...
TEST_GROUP_RUNNER(TestDiskEmulator) {
  ...
  RUN_TEST_CASE(TestDiskEmulator, write_block__succeed_if_buffer_size_is_less_than_BLOCK_SIZE);
...
TEST(TestDiskEmulator, write_block__succeed_if_write_is_inside_limits_of_disk) {
	...
	buffer = create_string('a', BLOCK_SIZE - 1);
...
TEST(TestDiskEmulator, read_block__cannot_read_if_outside_disk_limits) {
	...
	read_buffer = create_string('a', BLOCK_SIZE - 1);
	expected_string = create_string('a', BLOCK_SIZE - 1);
	...
	TEST_ASSERT_EQUAL(-1, read_block(NUM_BLOCKS, read_buffer)); // invalid read after last block
	TEST_ASSERT_EQUAL_STRING(expected_string, read_buffer);
...
TEST(TestDiskEmulator, read_block__write_and_read_correctly_what_was_written) {
	...
	char * expected_block_1 = create_string('a', BLOCK_SIZE);
	char * expected_block_2 = create_string('b', BLOCK_SIZE);
	char * expected_block_3 = create_string('c', BLOCK_SIZE);

	char * block_1 = create_string('a', BLOCK_SIZE + 5);
	char * block_2 = create_string('b', BLOCK_SIZE + 5);
	char * block_3 = create_string('c', BLOCK_SIZE + 5);
	...
	// than the BLOCK_SIZE. Therefore we need to manually check that every character is correct for a given length.
	// We want to make sure it contains the right data and size of copied data is block size (not more)
	TEST_ASSERT_EQUAL(0, are_character_sequences_equal(expected_block_1, read_buffer, BLOCK_SIZE));
	...
	TEST_ASSERT_EQUAL(0, are_character_sequences_equal(expected_block_2, read_buffer, BLOCK_SIZE));

	write_block(NUM_BLOCKS - 1, block_3, strlen(block_3) + 1);
	TEST_ASSERT_EQUAL(0, read_block(NUM_BLOCKS - 1, read_buffer));
	TEST_ASSERT_EQUAL(0, are_character_sequences_equal(expected_block_3, read_buffer, BLOCK_SIZE));
...
TEST(TestDiskEmulator, write_block__succeed_if_buffer_size_is_less_than_BLOCK_SIZE) {
	char * buffer;
	char read_buffer[BLOCK_SIZE];
...
TEST(TestDiskEmulator, write_block__we_can_write_any_data_structure_to_disk_and_read_them_correctly) {
	...
	void * read_buffer = malloc(BLOCK_SIZE * sizeof(void *));
	init_disk_emulator();
...
TEST(TestDiskEmulator, write_block__we_can_write_several_consecutive_data_structures_in_same_block_and_read_them_correctly) {
	...
	void ** write_buffer = malloc(BLOCK_SIZE * sizeof(void *));
	void ** read_buffer = malloc(BLOCK_SIZE * sizeof(void *));
	init_disk_emulator();
	...
	write_block(0, write_buffer, BLOCK_SIZE);
	read_block(0, read_buffer);
